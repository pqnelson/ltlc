...
Fixpoint dename' (t: PrettyTerm.pterm) (binds: list (string * nat)) : exp :=
  ...
  | PrettyTerm.Lam s t b => ELam (dename' t (hide s binds)) (dename' b (hide s binds))
...
Fixpoint FV_binds (identifiers: list string) (binds: list (string * nat)) : list (string * nat) :=
  match identifiers with
  | nil => binds
  | x::tl => match lookup x binds with
             | Some n => FV_binds tl binds
             | None => FV_binds tl (cons (x, length binds) binds)
             end
  end.

Fixpoint context_binds' (Gamma: PrettyTerm.context) (binds: list (string * nat)) : list (string * nat) :=
  match Gamma with
  | nil => binds
  | (x, _)::tl => match lookup x binds with
                  | Some n => context_binds' tl binds
                  | None => context_binds' tl (cons (x, length binds) binds)
                  end
  end.

Definition context_binds (Gamma: PrettyTerm.context) : list (string * nat) :=
  context_binds' (rev Gamma) nil.
...
Example context_binds_ex2 :
  context_binds (cons ("f", PrettyTerm.tau) (cons ("g", PrettyTerm.tau) (cons ("f", PrettyTerm.tau) nil)))
  = ("g", 1) :: ("f", 0) :: nil.
Proof.
  reflexivity.
Qed.
...
Example deconstruct_context_ex1 :
  let c := (snoc ("f"%string, `"tau") 
                 (cons ("h"%string, \"g" `"tau" ~> `"f") nil))
  in deconstruct_context c (context_binds c)
  = (ELam tau (EVar 1)) :: tau :: nil.
Proof. reflexivity. Qed.
...
Definition dename_with_context (c: PrettyTerm.context)
                               (t: PrettyTerm.pterm)
                               : context * exp
:= let binds := (FV_binds (PrettyTerm.FV t) (context_binds c))
   in (deconstruct_context c binds, dename' t binds).

Open Scope string_scope.
Example dename_with_context_ex1 :
  dename_with_context (cons ("g", PrettyTerm.tau) nil)
                      (`"f" $ `"g" $ `"f")
